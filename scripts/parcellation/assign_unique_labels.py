...
Parameters:
    ...
    --csv-path PATH    Destination lookup CSV (default: const.LABEL_LOOKUP_PATH).
...
def assign_unique_labels(rois_dir: Path, csv_path: Path, dry_run: bool = False) -> None:
    """Rename every roi_*.nii.gz file with a globally unique sequential ID and write a lookup CSV."""
    ...
    for label_dir in label_dirs:
        ...
        label_name = const.LABEL_DICT.get(label_number, f"Unknown ({label_number})")

        parcels = sorted(
            label_dir.glob("roi_*.nii.gz"),
            key=lambda p: int(p.name[4:-7]),
        )
        if not parcels:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rois-dir", type=Path, default=const.ROIS_DIR)
    parser.add_argument("--csv-path", type=Path, default=const.LABEL_LOOKUP_PATH)
